...
- [ ] **P3b** dashboard-as-host (unify `DashboardBlockSwitch` → `BlockRegistry.Render`, lights up `host_type='dashboard'`) · **`entity.map_id` backfill→bindings + column drop** (deferred span-the-layers migration) · **P4** create-or-pick UI + `EntityType.hosts_widget_type` + "Calendars subcategory" wizard.
