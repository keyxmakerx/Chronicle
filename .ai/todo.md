...
- [ ] **Consolidated addon-gated Characters page** — Players section (player-character-claiming) + NPC section (npcs); page absent if neither. Fold the NPC Gallery in: feature-tag portrait row + full revealed list + DM reveal/hide. Retire the standalone `npcs` gallery page (keep the `npc_gallery` block working).
- [ ] **Auto-premake the "Player Characters" sub-type on addon enable** — hook `player-character-claiming` enable to create the sub-type (with `CharacterLayout()`) idempotently, instead of only unlocking manual creation.
