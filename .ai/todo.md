...
- [ ] **P4b** generic picker for worldstate/timeline/maps + migrate maps off the bespoke picker · `EntityType.hosts_widget_type` (entity-type-template UI, lights up the `entity_type` inheritance rung) · "Calendars subcategory" wizard.
- [ ] **P3b** dashboard-as-host (unify `DashboardBlockSwitch` → `BlockRegistry.Render`, lights up `host_type='dashboard'`) · **`entity.map_id` backfill→bindings + column drop** (deferred span-the-layers migration).
