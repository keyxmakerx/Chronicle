...
- [x] **Journal Button Removal & Category Nav Redesign** — Removed "Session Journal" topbar button (journal will evolve into expanded notes/Obsidian-like section). Redesigned sidebar drill panel: merged header + action bar into compact clickable row (category name opens full page, replacing "View All"), count as pill badge, inline "+" button. Entity tree now has folder/page icons, vertical guide lines, smooth collapse/expand CSS transitions, and distinct reorder vs reparent drag-and-drop feedback.
