...
- `layer.updated` → MapSync (logged, no Foundry scene mutation)
- `calendar.event.created/updated/deleted` → CalendarSync
- `calendar.date.advanced` → CalendarSync (year, month, day, hour, minute)
- `calendar.season.changed` → CalendarSync (id, name, color — fires when date crosses season boundary)
- `calendar.moon.phase_changed` → CalendarSync (moon_id, moon_name, phase_name, phase_position — per moon)
- `calendar.weather.changed` → CalendarSync (full weather state — fires on PUT /calendar/weather)
- `calendar.structure.updated` → CalendarSync (fires when months/weekdays/moons/seasons/eras change)
- `calendar.era.changed` → CalendarSync (id, name, color — fires when date crosses era boundary)
- `sync.status` → API client (connection state)

## CalendarSync Cached State

CalendarSync caches server-side computed state received via WebSocket:
- `_chronicleCalendar` — Full calendar structure, re-fetched on `calendar.structure.updated`
- `_currentWeather` — Latest weather state from `calendar.weather.changed`
- `_currentSeason` — Current season from `calendar.season.changed` (may be null)
- `_currentEra` — Current era from `calendar.era.changed`

When creating events from Foundry → Chronicle, the module now passes `color`, `icon`,
and `all_day` fields if available from Calendaria event data.
