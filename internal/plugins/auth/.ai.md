...
Core plugin — always enabled. All authentication, session management, and password security. Login, registration, logout, session validation, CSRF double-submit cookie, bcrypt password hashing. Every other plugin that needs a logged-in user imports `auth` for `RequireAuth`, `GetSession`, `GetUserID`.

## For AI sessions

### Key files
- `service.go` — `Login`, `Register`, `GetSession`, `ValidatePassword` (bcrypt)
- `middleware.go` — `RequireAuth(svc)` Echo middleware; returns 401 or redirect
- `session.go` — Redis-backed session store; 24h TTL
