...
- **Diagnostic interface exposed:** `campaigns.FoundryPresenceLookup` (consumed by foundry_vtt + the live presence-indicator on the map detail page). See "Foundry presence diagnostic" below.

## Foundry presence diagnostic

The campaigns plugin owns a small diagnostic interface backing the `GET /campaigns/:id/foundry-presence` JSON endpoint (public, no auth — surface for the in-app pill that says "Foundry connected / disconnected / never seen"). The shape:

- `campaigns.FoundryPresenceLookup` interface — exposes "is this campaign's Foundry module currently connected via WebSocket; when was it last seen".
- `Handler.SetFoundryPresence(...)` setter — wired by `internal/app/routes.go` to the WS hub at startup.
- `GET /campaigns/:id/foundry-presence` — returns `{connected, last_seen, never_seen}` JSON.

**Important distinction (NW-2.2 D2-cleanup, PR #342):** This is the SURVIVING diagnostic structure. A parallel `maps.FoundryPresenceLookup` + `MapViewData.FoundryPresence` chain — used to inject Foundry presence into the map detail page's templ context — was removed in D2-cleanup because the lazy-load fragment pattern (PR #338) replaced it. The campaigns-side diagnostic stays because it powers the live JSON endpoint, NOT a templ context injection.

Companion removed in D2-cleanup: `campaigns.FoundryModuleBanner` + `FoundryModuleBannerLookup` + `Handler.SetFoundryModuleBanner`. The banner UI is now lazy-loaded as an HTMX fragment per the show-banner-fragment route (`internal/wire/show_banner_route_test.go` AST-pins the owner-gate). The lookup interface no longer needs to traverse the campaigns handler.

NW-2.3 (open) will relocate the `/foundry-presence` endpoint into the foundry_vtt plugin per the long-term plugin-isolation goal; until then it lives here.
