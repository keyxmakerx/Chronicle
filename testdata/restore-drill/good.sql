-- testdata/restore-drill/good.sql -- a minimal, valid Chronicle-shaped dump.
-- Used by tools/test-restore-drill.sh to prove the PASS path actually
-- passes. Deliberately omits calendar_events to also exercise the
-- "plugin table absent -> skip, don't fail" branch on every run.
